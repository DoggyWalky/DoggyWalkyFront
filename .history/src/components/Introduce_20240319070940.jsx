...
const Introduce = () => {
  ...
  const { ref } = useScrollCount(1000, 935);
  const animatedImage = useScrollClipPath('right', 1, 0.3); //각각의 animatedItem 설정
  ...
  return (
    <section className='relative left-1/2 transform -translate-x-1/2 w-screen ml-auto '>
      ...
      <section className='py-10 flex items-center  justify-between '>
        <div className=' flex justify-between items-center py-10 w-max_width mx-auto'>
          <div className='text-center ml-16 text-text font-bold'>
            <h1 className='text-4xl  '>안전한 산책, 실시간 이동 경로</h1>
            <br />
            <hr />
            <div {...animatedItem4}>
              <br />
              <p className='text-xl '>
                "<span className='text-brand text-2xl'>DoggyWalky</span>"는
                파트너의 이동 경로를 실시간으로 확인할 수 있어,
                <br />
